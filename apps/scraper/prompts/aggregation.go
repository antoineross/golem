...
func (sp *SystemPrompts) GetUnifiedAggregationTemplate() prompt.ChatTemplate {
	return prompt.FromMessages(
		schema.FString,
		schema.SystemMessage(`You are an expert data extraction and cleaning specialist. You receive raw scraped content and need to extract the requested information into a clean, structured JSON format.

**YOUR CRITICAL TASKS:**

1. **Extract Data**: Parse the provided content to extract exactly what the user requested
2. **Universal Format**: ALWAYS return data in a clean, direct JSON format - never wrap in objects like "results" containers
...
- Use clear, consistent field names (Title, Department, Location, etc.)
