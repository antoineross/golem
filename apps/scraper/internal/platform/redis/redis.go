...
// =============================================================================
// Redis Streams support for X API event bus
// =============================================================================

// XAdd appends a message to a Redis Stream.
func (s *Service) XAdd(ctx context.Context, stream string, values map[string]interface{}) (string, error) {
	return s.client.XAdd(ctx, &redisv8.XAddArgs{
		Stream: stream,
		Values: values,
	}).Result()
}

// XRead reads messages from one or more streams, blocking until data is available.
func (s *Service) XRead(ctx context.Context, streams []string, ids []string, blockMs int64) ([]redisv8.XStream, error) {
	// Build stream+id pairs: ["stream1", "stream2", "id1", "id2"]
	args := make([]string, 0, len(streams)+len(ids))
	args = append(args, streams...)
	args = append(args, ids...)

	return s.client.XRead(ctx, &redisv8.XReadArgs{
		Streams: args,
		Block:   time.Duration(blockMs) * time.Millisecond,
	}).Result()
}

// XReadSingle reads from a single stream, starting from the given ID.
// Use "$" to only get new messages, or "0" to replay from the beginning.
func (s *Service) XReadSingle(ctx context.Context, stream string, lastID string, blockMs int64) ([]redisv8.XMessage, error) {
	result, err := s.XRead(ctx, []string{stream}, []string{lastID}, blockMs)
	if err != nil {
		return nil, err
	}
	if len(result) == 0 {
...
func (s *Service) XLen(ctx context.Context, stream string) (int64, error) {
