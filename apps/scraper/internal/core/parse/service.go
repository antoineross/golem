...
func (s *Service) processAndAggregateNode(ctx context.Context, input map[string]interface{}) (*ParseResult, error) {
	...
	if ctxInterface, ok := input["workflow_context"]; ok {
		...
		workflowCtx = &WorkflowContext{}
		if err := json.Unmarshal(ctxBytes, workflowCtx); err != nil {
			s.log.LogWarnf("failed to unmarshal workflow context: %v", err)
			workflowCtx = &WorkflowContext{}
		}
	} else {
		workflowCtx = &WorkflowContext{}
...
func (s *Service) processAllContentWithUnifiedLLM(ctx context.Context, rawContentList []RawContentItem, analysis *PromptAnalysis) (interface{}, *TokenUsage, error) {
	...
	outputFormat := "json"
	if analysis != nil && analysis.OutputFormat != "" {
	...
	// Apply format conversion if needed (convert JSON to requested format)
	finalData := cleanedData
	if analysis != nil && analysis.OutputFormat != "" && analysis.OutputFormat != "json" {
		s.log.LogInfof("Converting JSON output to requested format: %s", analysis.OutputFormat)
		converted, err := s.formatConverter.Convert(cleanedData, analysis.OutputFormat)
		if err != nil {
			s.log.LogErrorf("Format conversion to %s failed: %v, returning JSON", analysis.OutputFormat, err)
			s.publishLLMAggregationEvent(ctx, "format.conversion_error", fmt.Sprintf("Format conversion failed: %v", err))
		} else {
			finalData = converted
			s.log.LogInfof("Successfully converted output to %s format", analysis.OutputFormat)
			s.publishLLMAggregationEvent(ctx, "format.conversion_success", fmt.Sprintf("Converted to %s format", analysis.OutputFormat))
		}
	}

	return finalData, finalTokenUsage, nil
}
...
func (s *Service) publishProgressEvent(ctx context.Context, event, message string) {
	...
	if tracer, ok := ctx.Value("eino_tracer").(*EinoTracer); ok && tracer != nil {
		...
		if err := tracer.jobService.PublishJobTrace(ctx, tracer.jobID, traceEvent); err != nil {
			s.log.LogWarnf("failed to publish job trace: %v", err)
		}
	}
